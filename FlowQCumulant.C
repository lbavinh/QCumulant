...
void FlowQCumulant(TString inputFileName, TString outputFileName)
{
  ...
  TProfile *pCorrelator2EtaGap = new TProfile("pCorrelator2EtaGap","2nd order correlator with eta-gap, TPC", ncent, 0, ncent);
  TProfile *pCorrelator2 = new TProfile("pCorrelator2","2nd order correlator", ncent, 0, ncent);
  TProfile *pCorrelator4 = new TProfile("pCorrelator4","4th order correlator", ncent, 0, ncent);
 ...
 for (int i=0; i<npid; i++)
  ...
  TProfile *pCov24 = new TProfile("pCov24","Covariance(<2>,<4>)", ncent, 0, ncent);
  ...
  for (int i=0; i<npid; i++)
  {
    pCov22RedEtaGap_FHCal[i] = new TProfile2D(Form("pCov22RedEtaGap_FHCal_pid%i", i), Form("Covariance(<2>,<2'>) with eta-gap of %s (FHCal)", pidNames.at(i).Data()), npt, 0, npt, ncent, 0, ncent);
  ...
  for (int iEv=0; iEv<n_entries; iEv++)
  {
    ...
    { // Track loop
      ...
      if (pt < minpt || pt > maxpt || abs(eta)>eta_cut) continue;
      if (abs(recoTrack->GetDCAx()) > DCAcut) continue; // DCAx cut
      if (abs(recoTrack->GetDCAy()) > DCAcut) continue; // DCAy cut
      if (abs(recoTrack->GetDCAz()) > DCAcut) continue; // DCAz cut
      if (recoTrack->GetNhits() < Nhits_cut ) continue; // TPC hits cut
      float charge = recoTrack->GetCharge();

      int ipt = -1;
      for(int j=0; j<npt;j++){
        if(pt>=pTBin[j] && pt<pTBin[j+1]) ipt = j;
      }

      int fId=-1;
      if (recoTrack->GetTofFlag() != 0 && recoTrack->GetTofFlag() != 4){
        if(recoTrack->GetPidProbPion()   > 0.9 && charge>0) fId=1; // pion+
        if(recoTrack->GetPidProbKaon()   > 0.9 && charge>0) fId=2; // kaon+
        if(recoTrack->GetPidProbProton() > 0.9 && charge>0) fId=3; // proton
        if(recoTrack->GetPidProbPion()   > 0.9 && charge<0) fId=5; // pion-
        if(recoTrack->GetPidProbKaon()   > 0.9 && charge<0) fId=6; // kaon-
        if(recoTrack->GetPidProbProton() > 0.9 && charge<0) fId=7; // antiproton
      }
      ...
      { // Left TPC subevent selection
        if (charge>0){ // Inclusive positively charged hadrons
          px2Gap[1][ipt][0]+=cos2phi;
          py2Gap[1][ipt][0]+=sin2phi;
          mpGap[1][ipt][0]++;
        }
        if (charge<0){ // Inclusive negatively charged hadrons
          px2Gap[1][ipt][4]+=cos2phi;
          py2Gap[1][ipt][4]+=sin2phi;
          mpGap[1][ipt][4]++;
        }
        if (fId>0){ // Identified charged hadrons
    ...
    if (w2 != 0 && w4 != 0){
      ...
      for (int ipt = 0; ipt < npt; ipt++){
        for (int id=0;id<npid;id++){
          ...
          if (mp[ipt][id] == 0 || wred2[ipt][id]==0 || wred4[ipt][id]==0) continue;
...
// root -l -b -q FlowQCumulant.C+'("/weekly/lbavinh/lbavinh/PicoDst/runlist_test.list","test.root")'
// root -l -b -q FlowQCumulant.C+'("/weekly/parfenov/mpd_winter2019/mpd_prod/7.7gev/merged_new/picodst_merged_604.root","test.root")'