TProfile *PlotV2EtaSubIntegrated(TProfile3D* &prV2,
...
TProfile *PlotV2EtaSubDifferentialVersusPt(TProfile3D *const &prV2,
...
TProfile *PlotV2EtaSubDifferentialVersusEta(TProfile3D*const &prV2,
                           const double pt_low = 0.2,
                           const double pt_high = 3.0,
                           const double cent_low = 10,
                           const double cent_high = 40)
...
void PlotV2EtaSubEventPlane(TString inputFileName = "SecondRun.root")
{
  ...
  TProfile *prV2diffpt = PlotV2EtaSubDifferentialVersusPt(pr,10,40,1.5);
  ...
  TProfile *prV2diffpt4080 = PlotV2EtaSubDifferentialVersusPt(pr,40,80,1.5);
  ...
  c.cd(3);
  // v2 versus centrality, 0.2<pt<3.0 GeV/c, |eta|<1.5
  TProfile *prV2int = PlotV2EtaSubIntegrated(pr,0.2,3.0,1.5);
  ...
  c.cd(4);
  // v2 versus eta, 0.2<pt<3.0 GeV/c, 10-40%
  TProfile *prV2diffEta = PlotV2EtaSubDifferentialVersusEta(prClone, 0.2, 3.0, 10, 40);
  prV2diffEta->SetMarkerStyle(27);
  prV2diffEta->SetMarkerColor(kBlue + 3);
  prV2diffEta->GetYaxis()->SetRangeUser(0., 0.1);
  prV2diffEta->Draw();

  c.SaveAs("FlowEtaSubEP.pdf");
}